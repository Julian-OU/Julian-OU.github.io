...
$$\begin{aligned}
    t\sum_{i}a^\dag_{\vec{r_i}}b_{\vec{r_i}+\vec{\delta_1}}&=-\frac{t}{N}\sum_i\sum_k\sum_{k'}a^\dag_k e^{i\vec{k}\cdot \vec{r}}\cdot b_{k'}e^{-i\vec{k'}\cdot (\vec{r}+\vec{\delta_1})}\\
    &=-t\sum_k\sum_{k'}a^\dag_k  b_{k'}e^{-i\vec{k'}\cdot \vec{\delta_1}} \cdot \frac{1}{N}\sum_ie^{i\vec{r_1}(\vec{k}-\vec{k'})}\\
