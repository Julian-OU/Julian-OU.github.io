...
\end{aligned}$$
其中$S$为样品区域，即支持区域，也就是图\ref{p3}(d)中央$N \times N$区域，$S_0$为零密度区域。使用过采样的方法不仅可以在傅里叶空间获取更多的高频信息，还可以在过采样的零密度区域进行约束\cite{Miao1998Phase}[[J. Opt. Soc. Am. A 15, 1662 (1998)](https://doi.org/10.1364/josaa.15.001662)]。
...
#### ER算法
GS算法可以进一步推广，广义的GS算法可用于约束条件在两个空间都是已知的情况，即在实空间和傅里叶空间来回转换，并在返回另一个空间前满足其约束。 1980年Fienup把GS算法推广到CDI中，得到误差下降算法（Error-Reduction, ER）\cite{Fienup1980Iterative,Fienup1984Reconstruction}[[Opt. Eng. 19, 193297 (1980)](https://doi.org/10.1117/12.7972513), [SPIE Process. Vol. 373, 147-160 (1948)](https://doi.org/10.1117/12.934545)]。ER算法的整体结构与GS算法相同，仅对约束条件(\ref{15})式进行了替换，
